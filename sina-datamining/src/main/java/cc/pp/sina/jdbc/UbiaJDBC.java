...
public class UbiaJDBC implements DataBaseConstant {
	...
	public void insertTodayResult1(String tablename, String usernameday, String username, String date, int followerscount,
			int friendscount, int statusescount, int followersadd, int friendsadd, int statusesadd, 
			String followersaddratio, String friendsaddratio, String statusesaddratio) throws SQLException {
		long time = System.currentTimeMillis()/1000;
		String sql = new String("INSERT INTO " + tablename + " (`usernameday`,`username`,`date`,`followerscount`," +
				"`friendscount`,`statusescount`,`followersadd`,`friendsadd`,`statusesadd`,`followersaddratio`," +
				"`friendsaddratio`,`statusesaddratio`,`lasttime`) VALUES (\"" + usernameday + "\",\"" + username + "\",\"" 
				+ date + "\"," + followerscount + "," + friendscount + "," + statusescount + "," + followersadd + "," +
				friendsadd + "," + statusesadd + ",\"" + followersaddratio + "\",\"" + friendsaddratio + "\",\"" +
				statusesaddratio + "\"," + time + ")");
		Statement statement = this.conn.createStatement();
		statement.execute(sql);
	...
	public void insertT2Users(String bind, String username, String accesstoken, String expires) throws SQLException {
		
		String sql = new String("INSERT INTO `sinausers` (`bid`,`username`,`accesstoken`,`expires`) " +
				"VALUES (" + bind + ",\"" + username + "\",\"" + accesstoken + "\"," + expires + ")");
		Statement statement = this.conn.createStatement();
		statement.execute(sql);
		statement.close();
	}
