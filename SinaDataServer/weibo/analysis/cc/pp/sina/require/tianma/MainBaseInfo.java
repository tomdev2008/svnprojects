...
public class MainBaseInfo {
	...
	public void dumpBaseinfo2() throws WeiboException, NumberFormatException, IOException, SQLException {

		LocalJDBC myjdbc = new LocalJDBC("127.0.0.1", "root", "root");
		if (myjdbc.mysqlStatus()) {
			List<String> tokens = myjdbc.getAllTokens();
			myjdbc.createSinauserTable("pp_company_baseinfos");
			BufferedReader fr = new BufferedReader(new FileReader(new File("users/20140103_user_baseinfo")));
			String uid;
			int count = 0;
			while ((uid = fr.readLine()) != null) {

				System.out.println("Read at: " + count++);
				...
				try {
					if (userinfo.getId().length() > 5) {
						myjdbc.insertUsersInfo("sinauserinfo_20140103", userinfo.getId(), userinfo.getScreenName(), 0,
								...
								userinfo.isVerified(), userinfo.getVerifiedType(), userinfo.getVerifiedReason(), 0);
					} else {
						myjdbc.insertUsersInfo("sinauserinfo_20140103", uid, "", 0, "", "", 0, 0, 0, 0, false, -1, "",
								0);
					}
				} catch (RuntimeException e) {
					myjdbc.insertUsersInfo("sinauserinfo_20140103", uid, "", 0, "", "", 0, 0, 0, 0, false, -1, "", 0);
				}
			}
			fr.close();
			myjdbc.sqlClose();
	...
	public void dumpBaseinfo1() throws WeiboException, NumberFormatException, IOException, SQLException {

		LocalJDBC myjdbc = new LocalJDBC("127.0.0.1", "root", "root");
		if (myjdbc.mysqlStatus()) {
			List<String> tokens = myjdbc.getAllTokens();
			myjdbc.createSinauserTable("sinauserinfo_20140103");
			BufferedReader fr = new BufferedReader(new FileReader(new File("users/20140103_user_baseinfo")));
			String uid;
			int count = 0;
			while ((uid = fr.readLine()) != null) {
	...
	public void dumpBaseinfo() throws WeiboException, NumberFormatException, IOException, SQLException {
		...
		if (myjdbc.mysqlStatus()) {
			List<String> tokens = myjdbc.getAllTokens();
			String[] cates = { "20140103_user_baseinfo" };
			for (String cate : cates) {
				...
				while ((str = fr.readLine()) != null) {
					...
					uid = str.split("\t")[0];
					usedcount = str.split("\t")[1];
					if ((Integer.parseInt(usedcount) > 100) && uid.length() > 5) {
