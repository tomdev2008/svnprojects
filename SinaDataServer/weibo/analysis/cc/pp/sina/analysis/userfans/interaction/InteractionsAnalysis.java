...
public class InteractionsAnalysis implements Serializable {
	...
	private static Logger logger = LoggerFactory.getLogger(InteractionsAnalysis.class);
	...
	public static void main(String[] args) throws Exception {
		...
		InteractionsAnalysis ia = new InteractionsAnalysis("127.0.0.1", "root", "root");
		ia.analysis();
		//		ia.moveResult();
		ia.closeMysql();
	}

	private final UfiJDBC myjdbc;

	public InteractionsAnalysis(String ip) {
		myjdbc = new UfiJDBC(ip);
		myjdbc.mysqlStatus();
	}

	public InteractionsAnalysis(String ip, String user, String password) {
		myjdbc = new UfiJDBC(ip, user, password);
		myjdbc.mysqlStatus();
	}
		...
		for (int i = 0; i < sinausers.size(); i++) {
			...
			try {
				...
				if (user.getStatusCode().equals("20003")) {
					...
					myjdbc.insertUserIteractions1(username + ufiUtils.getTodayDate(), username,
							ufiUtils.getTodayDate(), 0, json.toString());
					//					i++;
					continue;
				}
				...
				emotionratio.put("negative",
						Float.toString((float) Math.round(((float) remotions[0] / sum) * 10000) / 100) + "%");
				emotionratio.put("positive",
						Float.toString((float) Math.round(((float) remotions[1] / sum) * 10000) / 100) + "%");
				emotionratio.put("neutral",
						Float.toString((float) Math.round(((float) remotions[2] / sum) * 10000) / 100) + "%");

				JSONArray json = JSONArray.fromObject(emotionratio);

				myjdbc.insertUserIteractions1(username + ufiUtils.getTodayDate(), username, ufiUtils.getTodayDate(),
						allcount, json.toString());
			} catch (SQLException | WeiboException | IOException e) {
				//				e.printStackTrace();
				emotionratio.put("negative", "0.0%");
				emotionratio.put("positive", "0.0%");
				emotionratio.put("neutral", "0.0%");
				JSONArray json = JSONArray.fromObject(emotionratio);
				myjdbc.insertUserIteractions1(sinausers.get(i) + ufiUtils.getTodayDate(), username,
						ufiUtils.getTodayDate(), 0, json.toString());
				continue;
			} catch (RuntimeException e) {
				//				e.printStackTrace();
