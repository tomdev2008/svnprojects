...
public class AdvancedUserAnalysis implements Serializable {
	...
	public AdvancedUserAnalysis(String ip, String user, String password) {
	...
	public static void main(String[] args) throws WeiboException, SQLException {
		...
		AdvancedUserAnalysis ua = new AdvancedUserAnalysis("127.0.0.1", "root", "root");
		String[] result = ua.analysis("1747752902");
		ua.closeMysql();
	...
	public String[] analysis(String uid) throws WeiboException, SQLException {
		...
		/******************存储用户数据******************/
		//		usermysql.insertUserInfo(user.getId(), user.getName(), user.getLocation(), user.getProfileImageUrl(),
		//				user.getGender(), user.getFollowersCount(), user.getStatusesCount(), user.getFriendsCount(),
		...
		while ((cursor * 200 < user.getFollowersCount() + 200) && (cursor < 26)) {
			...
			for (User u : followers.getUsers())
			{
				...
				//						u.getGender(), u.getFollowersCount(), u.getStatusesCount(), u.getFriendsCount(),
				...
				top50fans = InsertSort.toptable(top50fans, u.getName() + "=" + fanssum);
				/***********7、Top50粉丝UID（按微博量）************/
				//				top50wb = InsertSort.toptable(top50wb, username + "=" + weibosum);
				top50wb = InsertSort.toptable(top50wb, u.getName() + "=" + weibosum);
				/******************8、水军分析******************/
				/***************9、Top50水军UID****************/
				/******************10、认证比例******************/
				if (addv == false) {
				...
				top50time = InsertSort.toptable(top50time, u.getName() + "=" + timediff);
				/******************平均每天发博量******************/
				averagewb = weibosum / timediff;
				/*********14、Top50粉丝UID（按平均发博量）***********/
				//				top50avgwb = InsertSort.toptable(top50avgwb, username + "=" + averagewb);
				top50avgwb = InsertSort.toptable(top50avgwb, u.getName() + "=" + averagewb);
				/******************15、认证分布******************/
				...
				top50active = InsertSort.toptable(top50active, u.getName() + "=" + fansactive);
			}
			cursor++;
		...
		for (int j = 1; j < province.length; j++) {
			if (province[j] != 0) {
				...
				result.getProvince().put(Integer.toString(j),
						Float.toString((float) Math.round(temp1 * 10000) / 100) + "%");
			}
		}
		/******************2、头像分析******************/
		...
		result.getGradeByFans().put("<100",
				Float.toString((float) Math.round(((float) fansclassfans[0] / existuids) * 10000) / 100) + "%");
		result.getGradeByFans().put("100~1000",
				Float.toString((float) Math.round(((float) fansclassfans[1] / existuids) * 10000) / 100) + "%");
		result.getGradeByFans().put("1000~1w",
				Float.toString((float) Math.round(((float) fansclassfans[2] / existuids) * 10000) / 100) + "%");
		result.getGradeByFans().put("1w~10w",
				Float.toString((float) Math.round(((float) fansclassfans[3] / existuids) * 10000) / 100) + "%");
		result.getGradeByFans().put(">10w",
				Float.toString((float) Math.round(((float) fansclassfans[4] / existuids) * 10000) / 100) + "%");
		/************5、用户等级分析（按微博量）*************/
		result.getGradeByWb().put("<100",
				Float.toString((float) Math.round(((float) fansclasswb[0] / existuids) * 10000) / 100) + "%");
		result.getGradeByWb().put("100~1000",
				Float.toString((float) Math.round(((float) fansclasswb[1] / existuids) * 10000) / 100) + "%");
		result.getGradeByWb().put("1000~5000",
				Float.toString((float) Math.round(((float) fansclasswb[2] / existuids) * 10000) / 100) + "%");
		result.getGradeByWb().put("5000~1w",
				Float.toString((float) Math.round(((float) fansclasswb[3] / existuids) * 10000) / 100) + "%");
		result.getGradeByWb().put(">1w",
				Float.toString((float) Math.round(((float) fansclasswb[4] / existuids) * 10000) / 100) + "%");
		/***********6、Top50粉丝UID（按粉丝量）************/
		for (int n = 0; n < 50; n++) {
		...
		/***********11、用户等级分析（按创建时间）*************/
		result.getGradeByTime().put("<1month",
				Float.toString((float) Math.round(((float) fansclasstime[0] / existuids) * 10000) / 100) + "%");
		result.getGradeByTime().put("1~6months",
				Float.toString((float) Math.round(((float) fansclasstime[1] / existuids) * 10000) / 100) + "%");
		result.getGradeByTime().put("6~12months",
				Float.toString((float) Math.round(((float) fansclasstime[2] / existuids) * 10000) / 100) + "%");
		result.getGradeByTime().put("12~18months",
				Float.toString((float) Math.round(((float) fansclasstime[3] / existuids) * 10000) / 100) + "%");
		result.getGradeByTime().put("18~24months",
				Float.toString((float) Math.round(((float) fansclasstime[4] / existuids) * 10000) / 100) + "%");
		result.getGradeByTime().put(">24months",
				Float.toString((float) Math.round(((float) fansclasstime[5] / existuids) * 10000) / 100) + "%");
		/**********12、Top50粉丝UID（按创建时间）************/
		for (int n = 0; n < 50; n++) {
		...
		for (int n = 0; n < 50; n++) {
			if (top50avgwb[n].length() > 5) {
				result.getTopsByAvgwb()
						.put(Integer.toString(n), top50avgwb[n].substring(0, top50avgwb[n].indexOf("=")));
			}
		}
		/******************15、认证分布******************/
		...
		/************16、用户等级分析（按活跃度）*************/
		result.getGradeByActive().put("<2",
				Float.toString((float) Math.round(((float) fansclassactive[0] / existuids) * 10000) / 100) + "%");
		result.getGradeByActive().put("2~4",
				Float.toString((float) Math.round(((float) fansclassactive[1] / existuids) * 10000) / 100) + "%");
		result.getGradeByActive().put("4~6",
				Float.toString((float) Math.round(((float) fansclassactive[2] / existuids) * 10000) / 100) + "%");
		result.getGradeByActive().put("6~8",
				Float.toString((float) Math.round(((float) fansclassactive[3] / existuids) * 10000) / 100) + "%");
		result.getGradeByActive().put(">8",
				Float.toString((float) Math.round(((float) fansclassactive[4] / existuids) * 10000) / 100) + "%");
		/************17、用户等级分析（按活跃度）*************/
		for (int n = 0; n < 50; n++) {
			if (top50active[n].length() > 5) {
				result.getTopsByActive().put(Integer.toString(n),
						top50active[n].substring(0, top50active[n].indexOf("=")));
			}
		}
		/***************18、该用户的活跃度*****************/
		float active = 0.00f;
		...
		inf = (float) (0.8 * (Math.log(ifollowers) / Math.log(1.5)) + 0.1
