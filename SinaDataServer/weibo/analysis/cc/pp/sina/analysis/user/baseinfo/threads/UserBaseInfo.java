	...
	public void run() {
		try {
			...
			try {
				...
				if (userinfo != null) {
					myJdbc.inserUserBaseinfo(tablename, userinfo.getId(), userinfo.getScreenName(), userinfo.getName(),
							...
							userinfo.getCreatedAt().getTime() / 1000, userinfo.isVerified(),
							userinfo.getVerifiedType(), userinfo.getAvatarLarge(), userinfo.getBiFollowersCount(),
							userinfo.getRemark(), userinfo.getVerifiedReason(), userinfo.getWeihao());
				} else {
