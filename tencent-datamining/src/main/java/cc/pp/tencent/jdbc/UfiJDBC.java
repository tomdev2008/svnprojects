...
public class UfiJDBC implements DataBaseConstant {
	...
	public HashMap<String,String> getT2Users() throws SQLException {
		...
		String sql = new String("SELECT `username`,`accesstoken`,`tokensecret` FROM `tencentusers`");
		Statement statement = this.conn.createStatement();
		ResultSet rs = statement.executeQuery(sql);
		while (rs.next()) {
	...
	public HashMap<Integer,String> getTencentUsers() throws SQLException {
		...
		int i = 0;
		while (rs.next()) {
			if (rs.getString("username").length() > 3) {
	...
	public HashMap<Integer,String> getIteractions(String date) throws SQLException {
		...
		HashMap<Integer,String> result = new HashMap<Integer,String>();
		String sql = new String("SELECT * FROM `tencentinteractions` WHERE `date` = " + date);
		Statement statement = this.conn.createStatement();
		ResultSet rs = statement.executeQuery(sql);
		int i = 0;
		String info = new String();
	...
	public HashMap<Integer,String> getAllIteractions() throws SQLException {
		...
		HashMap<Integer,String> result = new HashMap<Integer,String>();
		String sql = new String("SELECT * FROM `tencentinteractions`");
		Statement statement = this.conn.createStatement();
		ResultSet rs = statement.executeQuery(sql);
		int i = 0;
		String info = new String();
	...
	public void insertUserIteractions(String usernameday, String date, int allcount, String emotionratio) throws SQLException {
		long time = System.currentTimeMillis()/1000;
		String sql = new String("INSERT INTO `tencentinteractions` (`usernameday`,`date`,`allcount`,`emotionratio`," +
				"`lasttime`) VALUES (\"" + usernameday + "\",\"" + date + "\"," + allcount + ",\"" + emotionratio.replaceAll("\"", "\\\\\"")
				+ "\"," + time + ")");
		Statement statement = this.conn.createStatement();
		statement.execute(sql);
		statement.close();
	...
	public void insertUserIteractions1(String usernameday, String username, String date, int allcount, String emotionratio) throws SQLException {
		long time = System.currentTimeMillis()/1000;
		String sql = new String("INSERT INTO `tencentinteractions` (`usernameday`,`username`,`date`,`allcount`,`emotionratio`," +
